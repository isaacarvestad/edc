...
TEST(UnitFlow, CanMatchFastSimple) {
  UnitFlow::Graph uf(2, {{0, 1, 5}});
  uf.addSource(0, 5);
  uf.addSink(1, 5);
  uf.compute(10);
  auto matches = uf.matchingFast({0}, {1});
  EXPECT_EQ(matches, (std::vector<std::pair<int, int>>{{0, 1}}));
}
...
TEST(UnitFlow, WontMatchFastBeforeFlowComputed) {
  UnitFlow::Graph uf(2, {{0, 1, 5}});
  uf.addSource(0, 5);
  uf.addSink(1, 5);
...
/**
   ...
   single edge. We expect all edges on left match with one on the right.
 */
TEST(UnitFlow, CanMatchFastMultiple) {
  const int leftN = 10;
  const int rightN = 20;
  const int n = leftN + rightN;
  ...
  auto matches = uf.matchingFast(sources, targets);
...
/**
   ...
   Assuming edge capacities are large enough it should be possible to find two
   matchings using fast matching.
 */
TEST(UnitFlow, CanRouteAndMatchFastPathGraph) {
  const std::vector<UnitFlow::Edge> es = {{0, 1, 2}, {1, 2, 2}, {2, 3, 2},
                                          {3, 4, 2}, {4, 5, 2}, {5, 6, 2}};
  UnitFlow::Graph uf(7, es);
  uf.addSource(0, 1);
  ...
  auto matches = uf.matchingFast({0, 1}, {5,6});
  ASSERT_EQ((int)matches.size(), 2);
...
/**
   Construct and match the following graph using fast matching:

   s1 - t1 - s2
      \    /
        t2
 */
TEST(UnitFlow, CanRouteAndMatchFastDiamondGraph) {
  const std::vector<UnitFlow::Edge> es = {
      {0, 1, 2}, {0, 2, 8}, {1, 2, 10}, {1, 3, 1}, {2, 3, 10}};
  ...
  auto matches = uf.matchingFast({0, 3}, {1,2});
  ASSERT_EQ((int)matches.size(), 2);
}
...
/**
   ...
   and then find a matching for all source vertices using fast matching.
 */
 ...
 */
TEST(UnitFlow, CanRouteAndMatchFastKBipartite) {
  constexpr int layerSize = 100, k = 100;
  constexpr int n = layerSize * k;
...
/**
   Construct random graph with random capacities and find a matching using fast
   matching.
 */
TEST(UnitFlow, CanMatchFastLargeGraph) {
  for (int iteration = 0; iteration < 200; ++iteration) {
    ...
    uf.compute(INT_MAX);
    uf.matchingFast(sources, targets);
  }
}
